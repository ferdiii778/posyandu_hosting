...
class penanganan extends CI_Controller
{
    public function __construct()
    {
        ...
        // Memastikan header CORS diatur dengan benar agar mobile bisa mengakses
        header('Access-Control-Allow-Origin: *');
        header("Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS");
        header("Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With");

        // Menangani pre-flight request OPTIONS dari aplikasi mobile
        if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
            exit;
        }

        $this->load->database();
        $this->load->model(['modelsapi/Mpenanganan', 'modelsapi/Mbalita']);
    }
    ...
    private function _response($data, $status = 200) {
        // Membersihkan buffer untuk mencegah output HTML/Error PHP mengganggu JSON
    ...
    /**
     * 1. GET DATA DROPDOWN
     * Digunakan untuk mengisi dropdown Balita dan Tim Khusus di Mobile
     */
    public function get_form_resources()
    {
        // 1. Ambil identitas dari parameter GET yang dikirim aplikasi mobile
        $role = $this->input->get('role');
        $timsus_id = $this->input->get('timsus_id');
    
        // 2. Siapkan query dasar untuk mengambil Balita Stunting
        ...
        // 3. LOGIKA JOIN (Menggantikan Foreach di Website Mentor)
        // Jika login sebagai timsus, paksa join ke tabel detail_timsus agar data terfilter otomatis
        if ($role == 'timsus') {
        ...
        // 4. Ambil data timsus (Selalu diambil agar dropdown Tim di Admin tidak kosong)
        $data_timsus = $this->db->get('timsus')->result_array(); 
    
        // 5. Kirim response dengan struktur yang konsisten (menghindari error null di Flutter)
        $this->_response([
            'status' => true, 
            'data' => [
    ...
    public function index()
    {
        ...
        if ($id) {
            ...
            // Tentukan filter WHERE berdasarkan Role
            $where_role = ($role == 'timsus') ? array('treatment.timsus_id' => $timsus_id) : array();
    
            $preload = array(
                'join' => array(
                    array('pemeriksaan', 'pemeriksaan.kode_pemeriksaan = treatment.kode_pemeriksaan', 'left'),
                    array('balita', 'balita.nib = pemeriksaan.nib', 'left'),
                    array('timsus', 'timsus.timsus_id = treatment.timsus_id', 'left'),
                ),
    ...
    {
        // Mengambil input JSON dari mobile
        ...
        // Pastikan Model Mpenanganan->insert mengembalikan nilai ID atau boolean TRUE
        $insert = $this->Mpenanganan->insert($data);
        
        if ($insert) {
            $this->_response(['status' => true, 'message' => 'Data Treatment berhasil disimpan']);
        } else {
    ...
    /**
     * Mapping Input agar aman untuk PHP versi lama dan baru
     */
    private function _map_input($input) {
        // Ambil role dari input untuk validasi
        $role = isset($input['role']) ? $input['role'] : 'timsus';
        $timsus_id_login = isset($input['timsus_id_login']) ? (int)$input['timsus_id_login'] : 0;
        return [
            ...
            // Konversi Boolean ke Integer 1 atau 0 sesuai SQL int(1)
